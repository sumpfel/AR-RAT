...
class CameraLogic:
    ...
    def init_detector(self):
        # Path to model -> SENSORFUSION dir might be separate. 
        # But we need the model file. I should probably ensure it exists or point to the SENSORFUSION one.
        # Let's point to the SENSORFUSION directory for now to avoid copying binary files.
        # Original: model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'blaze_face_short_range.tflite')
        
        # Adjustable Path
        possible_paths = [
            os.path.join(os.path.dirname(os.path.abspath(__file__)), 'blaze_face_short_range.tflite'),
            '/home/christof/Dokumente/schule/2025_2026/inno_week/AR_RAT/SENSORFUSION/blaze_face_short_range.tflite'
        ]
        ...
        if not model_path:
            print("[CameraLogic] ERROR: blaze_face_short_range.tflite not found!")
            return
